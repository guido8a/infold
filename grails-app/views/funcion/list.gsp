...
<div class="btn-toolbar toolbar" style="margin-top: 5px">
    <div class="btn-group">
        <a href="#" class="btn btn-rojo btnNuevaFuncion">
    ...
    <table class="table-bordered table-condensed table-hover" width="100%">
        ...
        <g:if test="${funciones.size() > 0}">
            <g:each in="${funciones}" var="funcion">
                <tr data-id="${funcion?.id}" style="width: 100%">
                    <td style="width: 55%">${funcion?.descripcion}</td>
                </tr>
            </g:each>
        </g:if>
        <g:else>
...
<script type="text/javascript">
    ...
    function submitForm() {
        ...
        if ($form.valid()) {
            ...
            $.ajax({
                type    : "POST",
                url     : '${createLink(action:'saveFuncion_ajax')}',
                data    : $form.serialize(),
                success : function (msg) {
                    l.modal("hide");
                    if (msg == "ok") {
                        log("Función guardada correctamente","success");
                        setTimeout(function () {
                            location.reload(true);
                        }, 1000);
                    } else {
                        log("Error al guardar la función","error");
                    }
                }
            });
        } else {
    ...
    function deleteRow(id) {
        bootbox.dialog({
            ...
            message : "<i class='fa fa-trash fa-2x pull-left text-warning text-shadow'></i><span style='font-size: 14px; font-weight: bold'>&nbsp; ¿Está seguro que desea eliminar esta función?.</span>",
            buttons : {
                cancelar : {
                    label     : "<i class='fa fa-times'></i> Cancelar",
                    className : "btn-gris",
                ...
                aceptar : {
                    ...
                    callback  : function () {
                        $.ajax({
                            type    : "POST",
                            url     : "${createLink(controller: 'funcion', action:'eliminar_ajax')}",
                            data    : {
                                id:id
                            },
                            success : function (msg) {
                                if(msg == 'ok'){
                                    ...
                                    log("Error al eliminar la función","error")
                                }
                            } //success
                        }); //ajax
                    }
