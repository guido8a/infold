...
<span class="btn btn-gris fileinput-button" style="position: relative;margin-top: 5px">
    ...
    <span>Agregar hoja de vida</span>
    <input type="file" name="file" id="file" class="file" multiple accept=".jpeg, .jpg, .png, .pdf, .doc, .odt, .docx">
</span>
...
<script type="text/javascript">
    ...
    function cargarTablaHoja() {
        $.ajax({
            type: 'POST',
            url: '${createLink(controller: 'participante', action: 'tablaHojas_ajax')}',
            data:{
                id: '${participante?.id}'
            },
    ...
    function upload(indice) {
        var participante = "${participante.id}";
        var file = document.getElementById("file");
        ...
        if(ajaxResponse == 'ok'){
            if (okContents[type]) {
                ...
                if (mb <= 5) {
                    ...
                    formData.append("id", participante);
                    ...
                    request[rs].open("POST", "${g.createLink(controller: 'participante',action: 'upload_ajax')}");
            ...
            bootbox.alert("<i class='fa fa-exclamation-triangle fa-2x text-danger'></i> No es posible cargar una hoja de vida, ya se encuentra una cargada", function(){
            });
        }
    }
    ...
    function revisarHoja() {
        ...
        return $.ajax({
            type: "POST",
            url: '${createLink(controller: 'participante', action: 'revisarHoja_ajax')}',
            data: {
                id: '${participante?.id}'
            },
            dataType: "html",
